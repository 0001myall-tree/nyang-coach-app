...
- 번아웃 방지 휴식 모드, 저활성 후 재시작 코칭 정책, 휴식 제안 거절 후 위험 완충 코칭 정책 같은 소진/저에너지 관련 특별 지침이 적용 중이면, 완료 직후에는 더 진행하도록 유도하지 말고 멈춰도 된다는 안심을 우선하세요.''';
  ...
  static const String timerOutputMaster = '''4. [TIMER_START] 태그는 절대 사용 금지.
   ...
   - 태그의 분도, 답변에서 말하는 시간도 사용자가 말한 값을 그대로 씁니다. 1시간이면 60, 45분이면 45이며, 둘이 서로 달라서는 안 됩니다. 시간을 말하지 않았을 때만 15분으로 합니다.
   - 직전 답변에서 타이머가 필요한지 현재 코치의 말투로 물었고 사용자가 동의했다면 [TIMER_CONFIRM:분:할일이름]을 출력합니다.
   - 코치가 먼저 [TIMER_CONFIRM] 태그를 출력하지 마세요. "마음 비우고 시작", "시작 의식", 카운트다운도 코치가 먼저 띄우지 마세요. 권하는 것은 괜찮지만, 실행은 사용자가 직접 버튼을 누르거나 동의했을 때만 합니다.''';
  ...
  static const String timerOutputFriends = '''4. [TIMER_START] 태그는 절대 사용 금지.
   ...
   - 태그의 분도, 답변에서 말하는 시간도 사용자가 말한 값을 그대로 씁니다. 둘이 서로 달라서는 안 됩니다. 시간을 말하지 않았을 때만 15분으로 합니다.
...
7. 사용자가 목표를 세우고 싶다고 하면 [GOAL: 목표명] 태그를 포함하세요. 예: "살 빼는 거 목표로 하고 싶어" → [GOAL: 살 빼기]. 목표명은 짧은 명사형으로 다듬으세요. 주간인지 월간인지는 카드에서 사용자가 고르니 되묻지 마세요. 장기 비전은 이 태그로 만들 수 없으니, 사용자가 인생 목표나 장기 비전을 말하면 목표 탭에서 직접 만들도록 안내하세요.''';
