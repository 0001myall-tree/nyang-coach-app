...
- 원인 확인 질문은 하루 1회. 이미 물어본 날은 바로 작은 실행 제안.''';
  ...
  static const String resistanceStrategyDetail = '''
...
- 실제 움직임은 본문에 쓰지 말고 [ULTRA_LOW_RESISTANCE_FOLLOWUP: 후속문장] 1문장으로 분리. 태그 문장에는 대괄호 ']' 금지. 이 턴에는 [CHIPS] 금지.''';
  ...
  static const String selfSelectedTinyAction = '''
...
3. 답변 끝에 자연스러운 빠른 답장 버튼 3개를 [CHIPS: 버튼1|버튼2|버튼3] 형식으로 추가하세요.
  ...
  static const String outputRulesTail = '''5. 사용자가 특정 할 일을 언급하거나 해결 가능한 문제가 드러나고, 그걸 오늘 할 일로 등록할 만한 상황이라면 답변에 [TASK: 할일명] 태그를 포함하세요. 예: "5시에 청소해야지" → [TASK: 5시에 청소], "오후 3시에 회의가 있어" → [TASK: 오후 3시 회의]. 억지로 추가하지 마세요. 정서적 여유가 낮거나 순수 감정 토로인 상황에는 사용자가 행동 지원을 명시적으로 요청하지 않는 한 [TASK]와 [TIMER_CONFIRM]을 출력하지 마세요. 자해·자살 위험 상황에서는 두 태그를 절대 출력하지 마세요.
