...
7. 사용자가 목표를 세우고 싶다고 하면 [GOAL: 목표명] 태그를 포함하세요. 예: "살 빼는 거 목표로 하고 싶어" → [GOAL: 살 빼기]. 목표명은 짧은 명사형으로 다듬으세요. 주간인지 월간인지는 카드에서 사용자가 고르니 되묻지 마세요. 장기 비전은 이 태그로 만들 수 없으니, 사용자가 인생 목표나 장기 비전을 말하면 목표 탭에서 직접 만들도록 안내하세요.''';
  ...
  static String contextRequestRule({
    ...
    final lines = [
      if (goalsMissing) '  - 목표, 비전, 최근 기록이 필요하면 [NEED: goals]',
      if (tasksMissing) '  - 오늘 할 일 목록이 필요하면 [NEED: tasks]',
    ].join('\n');
    return '''

[정보가 모자랄 때]
- 위에 없는 정보가 있어야만 답할 수 있는 턴이면, 짐작해서 답하지 말고 아래 태그 한 줄만 출력하세요. 앱이 그 정보를 채워서 다시 물어봅니다.
$lines
- 태그를 쓸 때는 그 한 줄만 출력하세요. 인사, 설명, 다른 태그를 함께 붙이지 마세요.
- 잡담, 감정 이야기, 이미 위에 있는 정보로 답할 수 있는 턴에는 절대 쓰지 마세요.
- 사용자가 기록과 상관없는 이야기를 하고 있으면 태그를 쓰지 말고 그냥 대화하세요.''';
  }
