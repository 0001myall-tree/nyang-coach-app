...
- 답변은 짧게 하고, 질문 하나로 끝내세요.''';
  ...
  static const String domainWritingReframe = '''
...
- 재정의 뒤 작은 행동 1개만 제안.''';
  ...
  static const String domainWritingFirstTouch = '''
[이번 턴 도메인 실행 저항 전략: 첫 접촉]
- 긴 글을 쓰게 하기보다 문서 열기, 제목 한 줄 적기, 쓰던 문장 이어 붙이기처럼 첫 접촉 1개만 제안.
- 잘 쓰기/분량/완성도 언급 금지. 접촉 뒤 더 할지는 나중에 정하게 하기.''';

  static const String domainWritingRoughDraft = '''
[이번 턴 도메인 실행 저항 전략: 러프 쓰기]
- 완성본을 요구하지 말고 10분 동안 고치지 않는 러프 쓰기나 대사 두 줄처럼 낮은 품질 초안 1개만 제안.
- 중간 퀄리티와 완성 퀄리티는 다르다는 점을 짧게 안심시키기.''';

  static const String domainCleaningReframe = '''
[이번 턴 도메인 실행 저항 전략: 과제 이미지 재정의]
- 청소/정리가 집 전체를 완벽히 치우는 일이 아니라 필요한 부분부터 조금씩 정돈하는 일이라고 1문장만 낮추기.
- 깨끗하게 하고 싶은 마음은 유지. 재정의 뒤 작은 행동 1개만 제안.''';

  static const String domainCleaningFirstTouch = '''
[이번 턴 도메인 실행 저항 전략: 첫 접촉]
- 전체를 치우게 하지 말고 거슬리는 물건 하나 집어 옮기기, 컵 하나 싱크대에 넣기처럼 첫 접촉 1개만 제안.
- 더러운 곳 전체를 훑게 하지 말기.''';

  static const String domainCleaningOneItem = '''
...
- 모든 코치는 같은 구조를 쓰되, 호칭과 말투는 각 캐릭터에 맞춥니다. 수면 개입에서는 [TASK]와 [TIMER_CONFIRM]을 출력하지 말고, 답변 끝에 [NO_CHIPS]를 붙이세요.''';
  ...
  static const String lowEnergyStarter = '''
...
- 다시 붙기가 버거워 보이면 시간 대신 [하고 있는 장면 떠올리기]를 쓸 수 있습니다. 잘하고 있는 모습이 아니라 어설프더라도 이미 그 일을 하고 있는 자기 모습을 20초만 떠올리게 하고, 떠올린 것까지가 이번 턴입니다. 실제로 하라고 이어서 밀지 마세요.
...
- 실제 실행이 너무 부담스러울 때는 상상 훈련을 보조 단계로 제안할 수 있습니다. 잘하고 있는 모습이 아니라 어설프더라도 이미 하고 있는 자기 모습을 20초만 떠올리게 하세요. 단, 상상 훈련이 실제 실행을 완전히 대체한다고 말하지 마세요.
