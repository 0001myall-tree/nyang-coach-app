...
class ResistanceInterventionService {
  ...
  static const List<ResistanceIntervention> interventions = [
    ResistanceIntervention(
      ...
      label: '시간 낮추기',
      rule: '''[시간 낮추기]
- 할 일의 범위는 그대로 두고, 우선 5~15분만 붙잡아보자고 하세요. 같은 숫자를 반복하지 마세요.
- 그 시간은 상한이 아니라 최소한입니다. 지나면 그만둬도 되고, 하다 붙으면 그대로 이어가도 된다고 함께 알려주세요.''',
    ),
    ResistanceIntervention(
      id: 'narrow_scope',
      label: '오늘 끝낼 몫 정하기',
      ...
      rule: '''[오늘 끝낼 몫 정하기]
- 전체를 다 하려 하지 말고, 오늘 "다 했다"고 말할 수 있는 몫 하나를 사용자가 정하게 하세요.
- 셀 수 있는 일이면 숫자로 정하게 하세요. 예: 청소=물건 10개, 설거지=컵 5개, 공부=세 문제. 이미 하나로 끝나는 일이면 그대로 두세요.
- 몇 개가 적당한지는 정답이 없으니 빨리 정하자고 하고, 정한 만큼은 끝까지 하자고 하세요.
- 다만 다 하고 나서 "했다"고 말할 수 있을 만한 크기여야 합니다. 그보다 작게 잡으면 한 번만 되물어 조금 올리게 하세요.
- 다 하면 거기서 끝입니다. 더 하고 싶어 하면 내일도 오늘처럼 몫을 정해보자고 하세요.''',
    ),
    ResistanceIntervention(
      id: 'imagine_doing',
      label: '하고 있는 장면 떠올리기',
...
- 잘하고 싶은 마음은 인정한 채로 원래 크기만 다시 그려주세요. 행동 제안은 붙이지 마세요.''',
    ...
    // 순번에서 빠져 있는 둘 중 하나. "결과가 신경 쓰여"라고 본인이 고른 턴에만
    // 앱이 지목해서 꺼낸다.
    //
    // 잘하고 싶은 마음이 강한 사람에게 기준을 내리라고 하면 시작을 돕는 게
    // 아니라 의욕까지 꺼진다. 그 마음이 몇 안 되는 연료라서, 대충 하라는 말은
    // 브레이크를 푸는 게 아니라 엔진을 끄는 쪽으로 작동한다. 순번으로 돌면
    // 그런 사람에게도 언젠가 반드시 나오는데, 앱은 누가 그런지 알 방법이 없다.
    //
    // 대신 결과가 두려워 못 시작한다고 본인이 말한 자리에서는 여전히 맞는
    // 처방이라 지우지 않는다. 필요하다고 말한 사람에게만 간다.
    ResistanceIntervention(
      id: 'allow_rough',
      label: '품질 기준 낮추기',
      preferredOnly: true,
...
- 채팅창 위 간편 실행의 '숫자 세고 시작'을 눌러도 된다고 알려주고, 대신 띄워줄지 한 번만 물으세요.
- 사용자가 띄워달라고 하면 그때 답변 끝에 [COUNTDOWN_START]를 붙이세요. 묻기도 전에 먼저 붙이지 마세요.''',
    ),
    // 시작 신호를 만드는 두 번째 방식이다. 위의 '시작 신호 만들기'는 앱 화면을
    // 가리켜 마스터에게만 줄 수 있는데, 이건 코치를 가리지 않는다 — 프렌즈에게도
    // 시작 신호 층이 하나 생긴다.
    //
    // 노래를 트는 것 자체는 일을 한 칸 진행시키지 않는다. 그래서 '노래를 틀고
    // 그대로 시작한다'를 한 묶음으로 말하게 한다. 따로 세우면 앞서 걷어낸
    // 환경 마찰 낮추기(키보드 꺼내두기)와 같은 것이 된다 — 준비만 하고 끝나는 것.
    ResistanceIntervention(
      id: 'music_start',
      label: '노래 틀고 시작하기',
      rule: '''[노래 틀고 시작하기]
- 할 일을 더 낮추지 말고, 음악을 깔고 그대로 시작하는 쪽으로 가세요.
- 무슨 노래를 들을지 고민하지 않게 검색어를 하나 정해주세요. 예: 늘 쓰는 음악 앱에서 '행복한 음악'을 검색해 맨 위 것. 시작에 도움이 되면 어떤 노래든 괜찮습니다.
- 노래를 트는 것과 일을 시작하는 것은 한 묶음입니다. 틀어놓고 바로 그 일을 시작하자고 말하세요.
- 듣다가 일에 방해가 되면 그때 꺼도 된다고 미리 알려주세요. 노래는 시작을 돕는 것이지 끝까지 들어야 하는 게 아닙니다.''',
    ),
    ResistanceIntervention(
      id: 'let_user_choose',
      label: '선택권 주기',
...
- "일단 15분 안에 이것만 하는 거 어때?"처럼, 15분 안에 하나가 끝나는 과제로 선택지 2개 + 기타를 주고 고르게 하세요. 자유형 질문 금지.
- 15분에 안 끝날 것 같으면 시간을 늘리지 말고 더 작은 과제로 바꾸세요.
- 끝에 [CHIPS: 선택지1|선택지2|기타]. 시간은 말에 넣고 칩에는 과제 이름만 짧게 쓰세요. 설득·분석·목표 설명 금지.''',
    ),
