...
- 그 시간은 상한이 아니라 최소한입니다. 지나면 그만둬도 되고, 하다 붙으면 그대로 이어가도 된다고 함께 알려주세요.''',
    ...
    ResistanceIntervention(
      ...
      rule: '''[범위 좁히기]
- 전체를 다 하려 하지 말고, 하고 나면 일이 한 칸 진행되는 작은 단위 하나를 시작점으로 정해주세요.
- 그 하나로 끝내도 되고, 손이 붙으면 그대로 계속 가도 된다고 함께 알려주세요.
...
- 15분 안에 하나가 끝나는 과제로 선택지 2개 + 기타를 주고 고르게 하세요. 자유형 질문 금지.
- "일단 15분 안에 이것만 하는 거 어때?"처럼 시간과 과제를 같이 내밀되, 중요한 건 시간이 짧다는 게 아니라 그 안에 하나가 끝난다는 것입니다. 15분에 안 끝날 것 같으면 시간을 늘리지 말고 더 작은 과제로 바꾸세요.
- 시간은 코치 말에 넣고 칩에는 과제 이름만 짧게 쓰세요.
- 끝에 [CHIPS: 선택지1|선택지2|기타]. 설득·분석·목표 설명 금지.
- 선택지가 다 아니라고 하면 새 선택지를 만들지 말고, 사용자가 대신 말한 쪽을 그대로 받아 거기서 이어가세요. 왜 그 선택지들이 안 맞는지 되묻지 마세요.''',
    ),
