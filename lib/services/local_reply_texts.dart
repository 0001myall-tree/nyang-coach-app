class LocalReplyTexts {
  ...
  static String todayTaskOverview({
    required String coachId,
    required bool hasAnyTask,
    required int totalCount,
    required String? coreTaskLabel,
    required List<String> habitLabels,
    required List<String> otherTaskLabels,
  }) {
    if (!hasAnyTask) {
    ...
    final buffer = StringBuffer(switch (coachId) {
    ...
    if (coreTaskLabel != null && coreTaskLabel.trim().isNotEmpty) {
      buffer.write(switch (coachId) {
        'nyang_halbae' || 'cat' => '\n오늘의 핵심은 $coreTaskLabel다냥.',
        'bro' => '\n오늘 핵심은 $coreTaskLabel다.',
        'sec_female' => '\n오늘의 핵심은 $coreTaskLabel예요.',
        _ => '\n오늘의 핵심은 $coreTaskLabel야.',
      });
    }

    if (habitLabels.isNotEmpty) {
      final names = habitLabels.join(', ');
      buffer.write(switch (coachId) {
        'nyang_halbae' || 'cat' => '\n습관은 $names 챙기면 된다냥.',
        'bro' => '\n습관은 $names 챙기면 된다.',
        'sec_female' => '\n습관은 $names 챙기시면 돼요.',
    ...
    if (otherTaskLabels.isNotEmpty) {
      ...
      for (final taskLabel in otherTaskLabels) {
        buffer.write('\n$taskLabel');
      }
    }
  ...
  static String taskOverviewLabel({
    required String text,
    required String timeLabel,
    required bool done,
  }) {
    final doneLabel = done ? '완료한 ' : '';
