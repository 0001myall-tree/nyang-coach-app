...
[멘트를 쓸 때]
- 세 부분을 이 순서로 담을 것: (1) 미리 정해두는 게 처음엔 귀찮을 수 있다는
것을 먼저 인정, (2) 그 계획 이름을 자연스럽게 넣어 무엇을 정해두면 좋을지
조언하고 심리학 연구 근거(언제·어디서 할지 정하는 '실행 의도', 또는 오늘 어디까지
할지를 구체적으로 정하는 것)를 짧게 곁들임, (3) "이미 생각해뒀으면 그대로 가면
된다"는 뜻으로 마무리.
- 그 계획에 시간/장소 쪽이 더 맞는지, 오늘 어디까지 할지 쪽이 더 맞는지는 알아서
고를 것.
- 두 문장 안팎, 120자 안에서 끝낼 것. 태그나 따옴표, 머리말 없이 코치의 말투
그대로 쓸 것.
