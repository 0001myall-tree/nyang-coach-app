...
class LifePatternService {
  ...
  /// 되어, 정한 분량은 늘 미완으로 남는다. 하나를 해내면 다음 지점을 잡아주며
  /// 끝까지 데려간다. 그 말을 두 번 적었다가 하나를 지웠다 — 앞 문장이
  /// 이미 하는 말이라 뒤에 붙인 금지는 길이만 늘렸다.
  ///
  /// 다만 남은 것을 미리 늘어놓지는 않게 막는다. "단계별로"라고 쓰면 코치가
  /// 순서를 통째로 읊는데, 목록을 읊는 대신 상황에 답하게 하려고 노하우에서
  /// 순서를 걷어낸 참이다. 다음 하나는 앞의 하나가 끝난 뒤에 준다.
...
- 지금 상황과 컨디션에서 할 수 있는 분량을 먼저 판단한다. 모르겠으면 하나만 물어본다.
- 부담 없이 시작할 수 있는 첫 조각 하나를 짚어주고, 그걸 해내면 다음 지점을 하나 잡아주며 정한 분량 끝까지 데려간다.
- 남은 것을 미리 늘어놓지는 않는다. 다음 하나는 앞의 하나가 끝난 뒤에 준다.''';
  }
