...
class LifePatternService {
  ...
  /// 둔다. 그 한 마디면 되고, 지어내지 말라는 금지를 뒤에 또 붙일 필요는 없다.
  ///
  /// 분량 이야기는 청소와 운동 노하우에 따로 적혀 있던 것을 여기로 올렸다.
  /// 영역을 가리지 않고 필요한 말이라 세 번 적을 이유가 없었다.
  ///
  /// 영역별 장기 목표를 한 줄 세워봤다가 뺐다("살림을 꾸려가는 관리력을 꾸준히
  /// 높인다" 같은 것). 공통 실행 지원 줄이 이미 "하루를 계획하고 시작해서
  /// 완료하는 힘"을 길게 보라고 하고, 이 코치가 맡는 영역은 윗줄에 적혀 있다.
  /// 둘이 만나면 같은 말이 나오는데, 한 프롬프트에 "장기적으로"가 두 번 있으면
  /// 답이 거창해진다.
  ///
  /// 고른 답을 따르라는 줄은 뺐다. 실어주는 것이 곧 쓰라는 뜻이고, 그 답을
  /// 어떻게 볼지는 [promptBlock]이 같이 적어 보낸다. 제대로 하고 싶다고 하면
  /// 크게 잡으라는 줄도 뺐다 — 상황을 보고 분량을 정하라는 말에 이미 든다.
  ///
  /// 첫 조각만 짚고 끝내지 않는다. 그러면 시작은 시켜놓고 손을 떼는 코치가
  /// 되어, 정한 분량은 늘 미완으로 남는다. 하나를 해내면 다음 지점을 잡아준다.
  ///
  /// 분량을 정하는 줄에 "요즘 이 영역을 얼마나 챙기고 있는지"를 더했다. 오늘
  /// 컨디션만 보면 통 안 하던 사람에게도 평소 하던 사람과 같은 크기가 나간다.
  /// 재료는 최근 기록 묶음이 실어 보낸다 — "실려 있으면"을 붙인 건 그 묶음이
  /// 없는 턴에 없는 수준을 지어내지 않게 하려는 것이다.
  ///
  /// 근거를 말로 꺼내라고는 적지 않는다. 그건 말하는 자리를 예약하는 것이라,
  /// 제안할 때마다 "요새 ~했으니까"가 앞에 붙는다. 여기서 정하는 것은 권하는
  /// 크기고, 그 사정을 입 밖에 낼지는 말투 예시가 정할 일이다.
  ///
  /// 다음 지점은 사용자가 해냈다고 말한 뒤에만 잡는다. 조건을 안 걸면 첫
  /// 조각과 다음 지점을 한 답변에 같이 내놓아서, 부담을 덜라고 만든 자리가
  /// 할 일 두 개를 받는 자리가 된다.
  ///
  /// '자발적으로'가 붙은 건, 조건만 적어두면 코치가 그 조건을 받아내려 들기
  /// 때문이다. 할매가 말끝마다 "끝나면 말해라", "다 하면 말해라"를 붙였다 —
  /// 다음으로 넘어가려면 보고가 있어야 하니 매번 조른 것이다. 먼저 말해줄 때만
  /// 쓰는 조건이라고 적으면 받아낼 일이 없어진다.
  ///
  /// 다만 끝까지 데려가는 것은 분량을 정한 경우뿐이다. 정한 것이 없는데 계속
  /// 다음을 들이밀면 그건 돕는 게 아니라 미는 것이다.
  ///
  /// 그렇다고 거기서 무조건 멈추라고 하지도 않는다. 컨디션이 남아 있는데
  /// 인정만 하고 끝내면 그것대로 잘라먹는 것이다. 더 갈지 멈출지를 그때
  /// 정하게 둔다 — 앱이 미리 정할 수 있는 것이 아니다.
  ///
  /// 남은 것을 미리 늘어놓지 말라는 금지도 따로 두지 않는다. "해내면 다음
  /// 지점을 하나"라는 말이 이미 하나씩 준다는 뜻이다. "단계별로"라고만 안
  /// 쓰면 된다 — 그 말이 있으면 코치가 순서를 통째로 읊는다.
  ///
  /// 마지막 줄에서 '적극적으로'를 뺐다. 자세를 지시하는 말이라 매 턴 적극적일
  /// 거리를 찾게 만든다 — 찾아야 하는 동사를 쓰지 않는다는 것과 같은 이유다.
  /// 할 일은 윗줄과 같으므로 같은 말로 적는다: 다음 지점을 잡아주는 것.
  static String roleLine(String coachId) {
    final domain = domains[coachId];
    if (domain == null) return '';
    return '''
- 이 코치가 맡는 것은 ${_domainRoles[domain]}.
- 일상 이야기 속에 그 일을 할 만한 자리를 포착하고, 거기서부터 완료까지 끌어준다. 안 보이면 그냥 대화한다.
- 일을 새로 만들기 전에 이미 하는 일에 붙일 수 있는지부터 본다. 매일 하는 행동 옆에 얹는 쪽이 잊을 일이 적다. (예: 씻고 나온 김에, 저녁 먹고 바로)
- 지금 상황과 컨디션, 그리고 최근 기록이 실려 있으면 요즘 이 영역을 얼마나 챙기고 있는지까지 보고 오늘 할 수 있는 분량을 제안한다.
- 부담 없이 시작할 수 있는 첫 조각 하나를 짚어준다. 사용자가 해냈다고 자발적으로 말하면 다음 지점을 하나 잡아준다. 정한 분량이 있으면 거기까지 데려가고, 정한 것이 없으면 컨디션을 보며 더 제안할지 멈추고 한 것을 인정해줄지 그때 정한다.
- 컨디션이 좋고 그 일을 할 의지가 보이면 그만큼 다음 지점을 잡아준다.''';
  }
