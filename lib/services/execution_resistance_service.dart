...
class ExecutionResistanceService {
  ...
  ///
  /// '싫'을 소리 나는 대로 적은 '시러'도 여기 둔다. 목록이 전부 '싫'로 되어
  /// 있어서 "시러시러", "시러어어어어"가 통째로 빠져나갔다. 저항으로 안 잡히면
  /// 개입 로테이션이 안 돌고, 코치는 거절을 못 들은 채 같은 종류를 다시 민다.
  ///
  /// 다만 '시러'를 그냥 담으면 "드시러 갈까", "머리 자르시러"처럼 높임말 어미에
  /// 걸린다. 그쪽은 뒤에 반드시 다른 말이 따라오니(드시러 갈까) 뒤가 비었을
  /// 때만 센다. 겹쳐 쓰거나 늘여 쓴 것은 따로 잡는다.
