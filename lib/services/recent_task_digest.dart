...
class RecentTaskDigest {
  ...
  static String promptBlock(String? historyRaw, {DateTime? now}) {
    ...
    buffer.writeln('*이 중 네가 맡는 영역의 것만 보고 말하세요. 나머지는 아는 척하지 마세요.');
    // 숫자를 어떻게 읽을지만 적는다. "권할 때 근거를 한 마디 짚어라"까지 적으면
    // 제안하는 자리마다 "요새 ~했으니까"가 예약된다. 근거를 꺼내는 화법은
    // 코치별 말투 예시가 맡는 자리다.
    //
    // 뒷문장은 빈칸을 읽는 법이다. 정말 안 한 사람은 목록에 뜨지도 않는데,
    // 없다는 사실이 어디에도 안 적혀 있어서 침묵과 구분되지 않았다.
    buffer.writeln('*데이터를 읽고 요즘 한 빈도를 추론합니다. 네 영역 것이 여기 없으면 요즘 손을 안 댄 것입니다.');
    buffer.writeln(
