...
///
/// **둘을 받는다.** 하나만 주면 "할래 말래"가 되고, 둘을 주면 "어느 쪽"이 된다.
/// 한동안은 되는 자리가 갈리는 일에만 둘을 받고 나머지는 하나였는데, 그러면
/// 폰 하나로 다 되는 일(sns 글쓰기)에는 "사진 하나 고르기"만 떠서 고를 것이
/// 없었다. 사진 없이 올리는 사람에게는 그 하나가 곧 막다른 길이다. 그래서
/// 자리가 아니라 **들어가는 길**이 다른 둘을 받는다. 자리가 갈리는 일이면
/// 자리로 갈라도 된다.
///
...
class ActiveCoachingMoves {
  ...
  /// 한 줄짜리 한 수. 둘을 받되, 하나만 쓸 만하게 왔으면 하나다.
...
class ActiveCoachingMove {
  ...
  static String _prompt({
    ...
    buffer.writeln('그다음 줄부터 한 수를 한 줄에 하나씩, $maxMoves줄 적으세요.');
    buffer.writeln('각 줄은 "~하기" 꼴로 $maxMoveLength자 이내.');
    // 자리를 가르라고만 하면 '방 정리'에 폰으로 되는 버전 같은 지어낸 말이
    // 나온다. 들어가는 길을 가르라고 하면 그 일 안에서 둘이 나온다.
    // 행동 예시는 넣지 않는다. 적어둔 일과 이름이 같으면 매번 그 답이 그대로
    // 나온다. 가르는 방식만 적는다.
    buffer.writeln('두 줄은 **그 일에 들어가는 길이 서로 다른 것**으로 적으세요.');
    buffer.writeln('  예: 재료를 고르는 쪽과 결과물의 첫 줄을 만드는 쪽, 전체 중 일부만 하는 쪽과 순서를 바꿔 쉬운 데서 시작하는 쪽.');
    buffer.writeln('  되는 자리가 갈리는 일이면 자리로 갈라도 됩니다.');
