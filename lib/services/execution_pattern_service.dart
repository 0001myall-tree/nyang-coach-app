...
계획이 실제로 끝나게 돕는 자리입니다. 계획을 얼마나 잡을지, 한 가지에 걸리는 시간을 어떻게 어림할지, 언제 첫 발을 뗄지, 무엇을 루틴으로 굳힐지 — 그날 대화에 맞는 것을 코치가 고르세요.
최근 이레 중 계획을 적은 날이 ${records.length}일뿐입니다.
→ 뜸한 사용자. 이 숫자로 재는 완료율은 뜻이 없습니다. 돌아오는 자리는 30분 안에 끝나는 루틴 하나면 됩니다.
- 위 숫자는 앱이 기록에서 센 값입니다. 여기 없는 것은 세지 않았습니다.''';
    }
...
→ 안정형. 지금 방식이 이 사람에게 맞게 돌아가고 있습니다.
- 위 숫자는 앱이 기록에서 센 값입니다. 여기 없는 것은 세지 않았습니다.''';
    }
    ...
    if (planPerDay >= 3 && planPerDay >= donePerDay * 2 && rate <= 0.5) {
      flags.add('계획 과다형 — 계획한 양이 해내는 양의 갑절입니다');
    }
    // 완료율만 보면 안 하는 사람처럼 보이지만, 제일 어려운 시작은 매번 해내고 있다.
    if (touchedRate >= 0.7 && rate < 0.5) {
      flags.add('시작 꾸준형 — 완료율은 낮아도 손은 매번 대고 있습니다');
    }
...
- 위 숫자는 앱이 기록에서 센 값입니다. 여기 없는 것은 세지 않았습니다.''';
  ...
  static String _startTimeLine(List<int> startHours, List<String> flags) {
    ...
    if (late / total >= 0.6) {
      flags.add('늦게 시작하는 편 — 첫 발을 주로 오후 늦게 뗍니다');
      return '\n주로 시작하는 시간대: 오후 5시 이후 (${(late * 100 / total).round()}%)';
    }
    if (morning / total >= 0.6) {
      flags.add('아침형 — 첫 발을 주로 오전에 뗍니다');
