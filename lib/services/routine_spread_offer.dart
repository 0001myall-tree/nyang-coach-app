...
class RoutineSpreadOffer {
  ...
  /// 한 번에 이만큼까지만 나눈다.
  ///
  /// 여러 개를 한꺼번에 바꾸자고 하면 그건 제안이 아니라 개편이다. 무엇이
  /// 어떻게 되는지 한눈에 안 들어오면 사용자는 "그대로 둘게"를 누른다.
  ///
  /// 둘이면 "이건 월수금, 저건 화목"으로 한 문장에 들어가고 머릿속에 그림이
  /// 그려진다. 셋부터는 읽으면서 앞의 것을 잊는다.
  ///
  /// 되돌리는 품도 대칭이 아니다. 받아들이는 것은 한 번 누르면 끝인데 되돌리려면
  /// 루틴 탭에서 하나씩 고쳐야 한다. 한 번에 바꾸는 개수가 곧 되돌릴 때의
  /// 품이라, 적을수록 마음 편히 눌러볼 수 있다.
  static const int maxAssignments = 2;
...
나눌 것이 하나도 없을 수도 있습니다. 전부 매일이어야 뜻이 서는 것들이거나, 나눠도 될지 판단이 서지 않을 때입니다. 그때는 태그 없이 한 문장으로만 답하세요 - 애매한 것을 굳이 나누는 쪽보다 그대로 두는 쪽이 낫습니다. 거르는 것이 거저 되는 일이라면 사용자가 이미 그렇게 했을 텐데, 매일로 걸어둔 데에는 이유가 있습니다.''';
  ...
  ///
  /// 나눌 것이 하나도 없으면 배정이 빈 제안을 돌려준다. 아무것도 못 돌려주는
  /// 것과는 다른 답이다 - 코치를 못 부른 것이 아니라, 불러서 봤더니 나눌 것이
  /// 없다는 판단이 선 것이다. 부르는 쪽이 이 둘을 갈라야 한다. 나눌 게 없다고
  /// 한 코치 위로 앱이 "이 중에 골라달라"를 띄우면 코치 말을 뒤집는 셈이다.
  @visibleForTesting
