...
class LifeContextService {
  ...
  static const Map<String, String> kinds = {
    ...
    // 이 갈래가 없을 때 교대 근무자는 'job'으로 세어졌다. 그러면 코치는 낮에
    // 일하고 저녁에 쉬는 사람으로 알고 말하는데, 야간 근무자에게는 정반대다.
    // 아침에 자야 하는 사람에게 "저녁에 시간 나면"이라고 하는 셈이었다.
    //
    // 시각까지는 세지 않는다. 이 서비스가 흐릿하게 잡아서 잘 도는 것이라 —
    // 몇 시에 끝나는지는 야근·반차·시프트 변경에 매번 흔들리고, 정작 오늘
    // 몇 시인지는 사용자가 일정에 직접 적을 수 있다.
    'shift': '교대나 야간 근무가 있음 (낮과 밤이 주기적으로 바뀔 수 있고, 낮에 자는 날이 있을 수 있음)',
  };
  ...
  static const Map<String, List<String>> _signals = {
    ...
    // '야간'만으로는 세지 않는다. 'job'에 이미 '야근'이 있어서 둘이 붙어
    // 다니는데, "야간에 뭐 좀 하려고" 같은 말은 근무 이야기가 아니다.
    // 근무를 가리키는 말과 짝지어진 것만 남긴다.
    'shift': [
      '교대',
      '야간 근무',
      '야간근무',
