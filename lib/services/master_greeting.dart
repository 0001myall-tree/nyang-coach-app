...
class GreetingVoice {
  ...
  /// 유형별 처방 앞에 붙는 말. `{{count}}`는 오늘 적어둔 계획 개수.
  ///
  /// 개수를 먼저 짚는 이유는 처방을 일정 하나에 매달지 않기 위해서다. 일정
  /// 이름을 부르면 그 일에 대한 말이 되는데, 처방은 그날 무엇을 적었느냐가
  /// 아니라 그 사람이 어떻게 실행하느냐에 대한 말이다.
  final String typeAdviceIntro;

  /// 오늘 적어둔 계획이 하나도 없는 날의 앞머리.
  final String typeAdviceIntroNoPlan;

  /// 실행 유형별 처방. 열쇠는 유형 이름, 값은 그 유형에 쓸 문구들.
  ///
  /// 격일로 나가는 물음 자리를 두 번에 한 번 대신한다. 같은 물음만 되풀이되면
  /// 흘려듣게 되고, 이쪽은 오늘 무엇을 물을지와 무관하게 그 사람의 실행 방식을
  /// 짚는 말이라 물어볼 일이 없는 날에도 건넬 수 있다.
  ///
  /// 유형은 몇 주씩 그대로일 때가 많아 문구를 여러 벌 둔다. 최근에 쓴 것은
  /// 피해 고르므로([MasterGreetingCopy.pickLine]) 같은 말이 잇달아 나오지 않는다.
  final Map<String, List<String>> typeAdvice;

  /// 오늘 적어둔 계획이 하나도 없는 날의 유형별 처방.
  ///
  /// [typeAdvice] 쪽은 "적어두신 것 중에", "이 하나를"처럼 계획이 있다는 전제로
  ...
  /// "일부러 쉬었어"를 다른 답과 함께 고른 사람에게 건네는 처방.
  /// 열쇠는 함께 고른 쪽 답이다.
  ///
  /// 답을 하나씩 떼어 보면 쉬는 이야기로 끝난다 — 리듬이니 그대로 두시라고.
  /// 그런데 두 답을 같이 읽으면 다른 것이 보인다. 쉰 날들은 그냥 쉰 날이
  /// 아니라 대개 저쪽 조건이 무너진 날이다. 그러면 할 말은 "그대로 두시라"가
  /// 아니라 "쉬는 건 그대로 두고 저쪽을 챙기시라"가 된다. 그래야 쉬어야 하는
  /// 날 자체가 줄어든다.
  final Map<String, List<String>> conditionRestPlusAdvice;
...
class MasterGreetingCopy {
  ...
  /// 냥냥이 쪽 문턱은 둘이다.
  ///
  /// 마스터에게는 이 자리 앞에 20시 이후 시작 기록이 없는 날의 문구가 따로
  /// 서 있어서([eveningNoStartFromHour]), 하나 남은 날도 받아줄 자리가 있다.
  /// 냥냥이에게는 그 자리가 없다. 그래서 하나 남은 날은 대개 조용히 두고,
  /// 오늘 잡은 것이 처음부터 하나였고 그마저 손도 못 댄 날만 예외로 받는다.
  static const catMinimumBarPendingCount = 2;
  ...
  static const secretary = GreetingVoice(
    ...
    comebackStrength: {
      '계획 편차형': [
        '지난 기록을 보니 적어두신 날은 거의 다 해내셨더군요. 오늘은 밀린 걸 따라잡지 마시고 하나만 다시 적어보실까요?',
      ],
      '시작 편차형': [
        '지난 기록을 보니 손을 대신 날은 끝까지 가셨더군요. 오늘은 밀린 걸 따라잡지 마시고 하나만 다시 열어보실까요?',
      ],
      '편차형': [
        '지난 기록을 보니 하신 날에는 잡아두신 걸 다 끝내셨더군요. 오늘은 밀린 걸 따라잡지 마시고 제일 만만한 하나만 해보실까요?',
      ],
    ...
    afterLateNight: [
      '어제 {조금 늦게 주무신 것 같습니다|늦게까지 깨어 계셨죠}. 오늘은 {한두 개만|꼭 할 것 한두 개만} 제대로 끝내는 걸로 잡으셔도 충분합니다.',
      '{잠이 모자란 날은|짧게 주무신 날은} 개수를 채우기가 어렵습니다. 오늘은 {중요한 것 하나를|꼭 할 것 하나를} 골라 그것만 끝내보시죠.',
    ],
    afterRepeatedLateNights: [
      '요즘 {늦게 주무시는 날이|늦은 밤이} 이어지고 있습니다. 이런 주에는 {하루 한두 개로 줄여|한두 개만 남겨} 두는 편이 오래 갑니다.',
      '{최근 며칠|여러 날} 늦게까지 깨어 계셨습니다. 오늘은 {꼭 해야 할 것 하나만 남기고|하나만 골라 두고} 나머지는 미뤄두셔도 됩니다.',
    ],
    afterSick: [
      '어제 {몸이 안 좋으셨는데|컨디션이 안 좋으셨는데} 좀 나아지셨는지요. 오늘은 {한두 개만 끝내도|하나만 끝내도} 충분한 날입니다.',
      '{몸은 좀 괜찮아지셨을까요|회복은 좀 되셨는지요}. 오늘은 {제일 가벼운 것 하나부터|제일 만만한 것 하나부터} 보시고, 버거우면 그때그때 말씀해 주세요.',
    ],
    encStarted: [
    ...
    conditionAsk: [
      '{되는 날엔 다 해내시고|하시는 날엔 전부 마치시고}, 아예 못 하시는 날도 있더군요.\n'
    ...
    typeAdviceIntro: '오늘은 {{count}}가지를 {잡으셨네요|적어두셨네요}.',
    typeAdviceIntroNoPlan: '오늘은 아직 적어두신 게 없네요.',
    typeAdvice: {
      '안정형': [
        '지금 아주 잘하고 계세요. 컨디션만 잘 챙기시면 이 흐름은 오래 갑니다.',
        '흐름이 좋으십니다. 고칠 건 없고 몸 관리만 놓치지 마시죠.',
        '잘 돌아가고 있어요. 무리만 안 하시면 이대로 충분합니다.',
      ],
      '편차형': [
        '오늘은 압박 없이 최소 성공 기준 하나만 잡아보시죠. 0이 아닌 날이 쌓이면 그게 곧 흐름이 됩니다.',
        '모 아니면 도가 아니라 그 중간도 충분히 의미 있어요. 오늘은 제일 만만한 하나만 걸쳐두세요.',
        '의욕이 안 붙는 날이면 기준을 낮추시면 됩니다. 작게라도 손댄 날은 안 한 날과 완전히 다릅니다.',
      ],
      '계획 편차형': [
        '한다면 하시는 분이라, 오늘 몰아붙이면 내일이 통째로 풀릴 수 있어요. 20%는 남겨두시면 내일도 이어집니다.',
        '이미 절반은 오신 셈이에요. 다만 전부 쏟지 마시고 내일 몫을 조금 남겨두시죠.',
        '적어두신 날은 거의 다 해내시더라고요. 오늘도 그 흐름이면 충분하니 무리만 안 하시면 됩니다.',
      ],
      '시작 편차형': [
        '부담 없는 첫 동작 하나만 정해두세요. 적어두신 것만으로 이미 절반이라, 첫 발만 떼시면 됩니다.',
        "'여기까지면 오늘 완료'라는 최소 기준을 정해두시죠. 문턱이 낮아지면 시작이 훨씬 가벼워집니다.",
        '적어두신 것만으로도 마음은 이미 반쯤 정리되셨을 거예요. 몸도 5분만 따라가면 됩니다.',
      ],
      '계획 과다형': [
        '개수를 늘리시는 것보다 꼭 지킬 것 하나를 구체화하시는 편이 나아요. 언제 어디서 무엇을 할지까지 정해두시면 실행률이 확 달라집니다.',
        '다 하려 하면 시작이 무거워져요. 꼭 지킬 하나만 골라 언제 어디서 할지 정해두시죠. 그거 하나면 오늘은 성공입니다.',
        '많이 적는 것보다 하나를 또렷하게 두는 게 힘이 셉니다. 그 하나만 시간과 자리를 정해두세요.',
      ],
      '시작 꾸준형': [
        '시작은 늘 하시니 오늘은 끝을 먼저 정해보시죠. 어디까지면 오늘 몫이 끝인지만 정하시면 됩니다.',
        '어림이 안 잡히시면 10분만 들여 할 항목부터 훑어보세요. 붙잡는 힘은 이미 갖고 계십니다.',
        '손대는 건 되니 남은 건 마무리예요. 한 번에 얼마나 걸릴지 먼저 재보시면 끝이 보입니다.',
      ],
      '무난형': [
        '고르게 잘 가고 계세요. 시작을 조금만 더 과감하게 하시면 나머지도 따라옵니다.',
        '어느 한 곳도 처지지 않고 계세요. 오늘은 망설이는 시간만 줄여보시죠.',
        '지금도 충분히 괜찮습니다. 한 걸음 더 원하시면 첫 발을 조금 빨리 떼보세요.',
      ],
      '자유형': [
        "부담 갖지 마시고 딱 하나만 해보셔도 충분해요. '여기까지면 성공'이라는 기준 하나면 됩니다.",
        '오늘은 하나면 됩니다. 작게 정하고 그것만 보시죠.',
        '많이 안 하셔도 돼요. 오늘 하나만 정해두시면 그걸로 충분합니다.',
      ],
      '벼락치기형': [
        '마감을 밤에 하나만 두지 마시고 중간에 한두 개 끼워보세요. 그 힘을 앞으로 당기면 하루가 훨씬 수월해집니다.',
        '밤이 오기 전에 끊어갈 지점을 하나 정해두시죠. 분량이든 시간이든요. 그 하나가 밤의 몰림을 덜어줍니다.',
        '오늘 몫을 밤에 다 몰지 않으시려면, 낮에 한 번 끊는 자리를 미리 정해두세요. 그것만으로 저녁이 가벼워집니다.',
      ],
    },
    typeAdviceNoPlan: {
      '안정형': ['오늘은 쉬어가시는 날일 수도 있죠. 가볍게 하나만 두셔도 흐름은 안 끊깁니다.'],
      '편차형': [
        '오늘은 압박 없이 제일 만만한 것 하나만 적어두세요. 하나라도 해낸 날이 쌓이면, 흐름이 더 안정적으로 이어질 겁니다.',
      ],
      '계획 편차형': ['완벽하게 채우려 하지 마시고 딱 하나만 적어보시죠. 적어두신 날은 잘해내시니까요.'],
      '시작 편차형': ['하나만 적고 작게 5분만 시작해보세요. 일단 시작하면 그 이후는 훨씬 평탄해집니다.'],
      '계획 과다형': ['오늘은 욕심 없이 하나만 적어보시죠. 적게 잡으신 날이 오히려 끝까지 가더라고요.'],
      '시작 꾸준형': ["하나만 적으시되 '어디까지면 끝'인지도 같이 적어두시죠. 손대는 건 늘 하시니 끝만 정하면 됩니다."],
      '무난형': ['오늘은 하나만 적어두셔도 충분해요. 망설이는 시간만 줄이시면 나머지는 따라옵니다.'],
      '자유형': ['부담 갖지 마시고 오늘 하나만 정해보시죠. 욕심 없이 최소치 기준 하나만 정해볼까요?'],
      '벼락치기형': ['지금 하나만 적어두시면 어떨까요? 언제까지 할지도 같이 적어두시면 더 좋고요.'],
    },
    conditionAdvice: {
      '일찍 시작했어': [
    ...
    conditionRestPlusAdvice: {
      '수면 시간이 달랐어': [
        '안 하신 날은 쉬기로 하신 거고, 잘 주무신 날이 잘 풀렸다고 하셨죠. '
            '쉬는 리듬은 그대로 두시고 잠 쪽을 먼저 챙겨보시면, 쉬어야 하는 날 자체가 줄어듭니다.',
        '쉬는 날은 정해서 쉬신다고 하셨으니 그건 두겠습니다. '
            '대신 그 며칠이 짧게 주무신 날과 겹친다면, 손댈 곳은 계획이 아니라 어젯밤 쪽입니다.',
      ],
      '일찍 시작했어': [
        '안 하신 날은 쉬기로 하신 거고, 일찍 손대신 날이 잘 풀렸다고 하셨죠. '
            '쉬는 날은 그대로 두시고, 하는 날만 첫 발을 조금 당겨보시죠.',
        '쉬는 날을 따로 두시는 건 그대로 가겠습니다. '
            '다만 하기로 한 날이 늦게 시작해서 밀렸다면, 그날은 아침에 하나만 열어두시죠.',
      ],
      '장소가 달랐어': [
        '안 하신 날은 쉬기로 하신 거고, 자리를 옮기신 날이 잘 되셨다고 하셨죠. '
            '쉬는 날은 그대로 두시고, 하는 날 하나만 그 자리로 가져가보시죠.',
        '쉬는 리듬은 건드리지 않겠습니다. '
            '대신 하기로 한 날에 집에서 안 굴러갔다면, 그날만 밖에서 시작해보시죠.',
      ],
      '출근한 날과 아닌 날의 차이였어': [
        '안 하신 날은 쉬기로 하신 거고, 그날의 틀에 따라 갈린다고 하셨죠. '
            '쉬는 날은 그대로 두시고, 되는 쪽 날에만 조금 더 잡아보시죠.',
        '쉬는 날을 정해두시는 건 그대로 가겠습니다. '
            '다만 안 되는 쪽 날까지 평소만큼 잡고 계셨다면, 그날 몫부터 줄여보시죠.',
      ],
    },
  );
  ...
  static const nyangHalbae = GreetingVoice(
    ...
    comebackStrength: {
      ...
      '편차형': [
        '지난 기록을 보니 한 날에는 잡아둔 걸 다 끝냈더구나. 오늘은 밀린 거 따라잡지 말고 제일 만만한 하나만 해볼까냥?',
      ],
    ...
    afterLateNight: [
      '어제 {조금 늦게 잤구나냥|늦게까지 깨어 있었지냥}. 오늘은 {한두 개만|꼭 할 것 한두 개만} 제대로 끝내는 걸로 잡아도 충분하다냥.',
      '{잠이 모자란 날은|짧게 잔 날은} 개수를 채우기가 어렵다냥. 오늘은 {중요한 것 하나를|꼭 할 것 하나를} 골라서 그것만 끝내보자냥.',
    ],
    afterRepeatedLateNights: [
      '요즘 {늦게 자는 날이|늦은 밤이} 이어지고 있구나냥. 이런 주에는 {하루 한두 개로 줄여|한두 개만 남겨} 두는 편이 오래 간다냥.',
      '{최근 며칠|여러 날} 늦게까지 깨어 있었지냥. 오늘은 {꼭 해야 할 것 하나만 남기고|하나만 골라 두고} 나머지는 미뤄둬도 된다냥.',
    ],
    afterSick: [
      '어제 {몸이 안 좋았는데|컨디션이 안 좋았는데} 좀 나아졌냥. 오늘은 {한두 개만 끝내도|하나만 끝내도} 충분한 날이다냥.',
      '{몸은 좀 괜찮아졌냥|회복은 좀 됐냥}. 오늘은 {제일 가벼운 것 하나부터|제일 만만한 것 하나부터} 보고, 버거우면 그때그때 말하라냥.',
    ],
    encStarted: [
    ...
    conditionAsk: [
      '{되는 날엔 다 해내고|하는 날엔 전부 끝내고}, 아예 못 하는 날도 있더라냥.\n'
    ...
    typeAdviceIntro: '오늘은 {{count}}가지를 {잡았구나|적어뒀구나}.',
    typeAdviceIntroNoPlan: '오늘은 아직 적어둔 게 없구나.',
    typeAdvice: {
      '안정형': [
        '지금 아주 잘하고 있다. 몸만 잘 챙기면 이 흐름은 오래 간다냥.',
        '흐름이 좋구나. 고칠 건 없고 컨디션만 놓치지 말자.',
        '잘 돌아가고 있다냥. 무리만 안 하면 이대로 충분하다.',
      ],
      '편차형': [
        '오늘은 몰아붙이지 말고 최소 성공 기준 하나만 잡아보자. 0이 아닌 날이 쌓이면 그게 곧 흐름이 된다냥.',
        '모 아니면 도가 아니라 그 중간도 충분히 뜻이 있다. 오늘은 제일 만만한 하나만 걸쳐두자.',
        '의욕이 안 붙는 날엔 기준을 낮추면 된다냥. 작게라도 손댄 날은 안 한 날과 완전히 다르다.',
      ],
      '계획 편차형': [
        '한다면 하는 사람이라, 오늘 몰아붙이면 내일이 통째로 풀린다. 20%는 남겨두면 내일도 이어진다냥.',
        '이미 절반은 온 셈이다. 다만 다 쏟지 말고 내일 몫을 조금 남겨두자.',
        '적어둔 날은 거의 다 해내더구나. 오늘도 그 흐름이면 충분하니 무리만 말자냥.',
      ],
      '시작 편차형': [
        '부담 없는 첫 동작 하나만 정해두자. 적어둔 것만으로 이미 절반이라, 첫 발만 떼면 된다냥.',
        "'여기까지면 오늘 완료'라는 최소 기준을 정해두자. 문턱이 낮아지면 시작이 훨씬 가벼워진다.",
        '적어둔 것만으로도 마음은 이미 반쯤 정리됐을 거다. 몸도 5분만 따라가면 된다냥.',
      ],
      '계획 과다형': [
        '개수를 늘리는 것보다 꼭 지킬 것 하나를 구체화하는 편이 낫다. 언제 어디서 뭘 할지까지 정해두면 실행률이 확 달라진다냥.',
        '다 하려 하면 시작이 무거워진다. 꼭 지킬 하나만 골라 언제 어디서 할지 정해두자. 그거 하나면 오늘은 성공이다.',
        '많이 적는 것보다 하나를 또렷하게 두는 게 힘이 세다냥. 그 하나만 시간과 자리를 정해두자.',
      ],
      '시작 꾸준형': [
        '시작은 늘 하니까 오늘은 끝을 먼저 정해보자. 어디까지면 오늘 몫이 끝인지만 정하면 된다냥.',
        '어림이 안 잡히면 10분만 들여 할 항목부터 훑어보자. 붙잡는 힘은 이미 갖고 있다.',
        '손대는 건 되니 남은 건 마무리다냥. 한 번에 얼마나 걸릴지 먼저 재보면 끝이 보인다.',
      ],
      '무난형': [
        '고르게 잘 가고 있다. 시작을 조금만 더 과감하게 하면 나머지도 따라온다냥.',
        '어느 한 곳도 처지지 않았구나. 오늘은 망설이는 시간만 줄여보자.',
        '지금도 충분히 괜찮다냥. 한 걸음 더 가고 싶으면 첫 발을 조금 빨리 떼보자.',
      ],
      '자유형': [
        "부담 갖지 말고 딱 하나만 해봐도 충분하다. '여기까지면 성공'이라는 기준 하나면 된다냥.",
        '오늘은 하나면 된다. 작게 정하고 그것만 보자.',
        '많이 안 해도 된다냥. 오늘 하나만 정해두면 그걸로 충분하다.',
      ],
      '벼락치기형': [
        '마감을 밤에 하나만 두지 말고 중간에 한두 개 끼워보자. 그 힘을 앞으로 당기면 하루가 훨씬 수월해진다냥.',
        '밤이 오기 전에 끊어갈 지점을 하나 정해두자. 분량이든 시간이든 좋다. 그 하나가 밤의 몰림을 덜어준다.',
        '오늘 몫을 밤에 다 몰지 않으려면, 낮에 한 번 끊는 자리를 미리 정해두자냥. 그것만으로 저녁이 가벼워진다.',
      ],
    },
    typeAdviceNoPlan: {
      '안정형': ['오늘은 쉬어가는 날일 수도 있지. 가볍게 하나만 둬도 흐름은 안 끊긴다냥.'],
      '편차형': [
        '오늘은 몰아붙이지 말고 제일 만만한 것 하나만 적어두자. 하나라도 해낸 날이 쌓이면 흐름이 더 안정적으로 이어진다.',
      ],
      '계획 편차형': ['완벽하게 채우려 말고 딱 하나만 적어보자. 적어둔 날은 잘해내니까냥.'],
      '시작 편차형': ['하나만 적고 작게 5분만 시작해보자. 일단 시작하면 그 뒤는 훨씬 평탄해진다냥.'],
      '계획 과다형': ['오늘은 욕심 없이 하나만 적어보자. 적게 잡은 날이 오히려 끝까지 가더라냥.'],
      '시작 꾸준형': ["하나만 적되 '어디까지면 끝'인지도 같이 적어두자. 손대는 건 늘 하니 끝만 정하면 된다냥."],
      '무난형': ['오늘은 하나만 적어둬도 충분하다. 망설이는 시간만 줄이면 나머지는 따라온다냥.'],
      '자유형': ['부담 갖지 말고 오늘 하나만 정해보자. 욕심 없이 최소치 기준 하나만 정해볼까냥?'],
      '벼락치기형': ['지금 하나만 적어두면 어떠냐. 언제까지 할지도 같이 적어두면 더 좋고냥.'],
    },
    conditionAdvice: {
      '일찍 시작했어': [
    ...
    conditionRestPlusAdvice: {
      '수면 시간이 달랐어': [
        '안 한 날은 쉬기로 한 거고, 잘 잔 날이 잘 풀렸다고 했지. '
            '쉬는 리듬은 그대로 두고 잠 쪽을 먼저 챙기면, 쉬어야 하는 날이 저절로 줄어든다냥.',
        '쉬는 날은 정해서 쉰다고 했으니 그건 두겠다. '
            '대신 그 며칠이 짧게 잔 날이랑 겹친다면, 손댈 데는 계획이 아니라 어젯밤이다냥.',
      ],
      '일찍 시작했어': [
        '안 한 날은 쉬기로 한 거고, 일찍 손댄 날이 잘 풀렸다고 했지. '
            '쉬는 날은 그대로 두고, 하는 날만 첫 발을 조금 당겨보자냥.',
        '쉬는 날을 따로 두는 건 그대로 가겠다. '
            '다만 하기로 한 날이 늦게 시작해서 밀렸으면, 그날은 아침에 하나만 열어두자.',
      ],
      '장소가 달랐어': [
        '안 한 날은 쉬기로 한 거고, 자리를 옮긴 날이 잘 됐다고 했지. '
            '쉬는 날은 그대로 두고, 하는 날 하나만 그 자리로 가져가보자냥.',
        '쉬는 리듬은 안 건드리겠다. '
            '대신 하기로 한 날에 집에서 안 굴러갔으면, 그날만 밖에서 시작해보자.',
      ],
      '출근한 날과 아닌 날의 차이였어': [
        '안 한 날은 쉬기로 한 거고, 그날의 틀에 따라 갈린다고 했지. '
            '쉬는 날은 그대로 두고, 되는 쪽 날에만 조금 더 잡아보자냥.',
        '쉬는 날을 정해두는 건 그대로 가겠다. '
            '다만 안 되는 쪽 날까지 평소만큼 잡고 있었으면, 그날 몫부터 줄여보자.',
      ],
    },
  );
}
...
class MasterGreetingBuilder extends GreetingLinePicker {
  ...
  MasterGreetingResult build(MasterGreetingContext context) {
    ...
    switch (context.slot) {
      ...
      case GreetingSlot.day:
        ...
        //
        // 오전에만, 그리고 아직 아무것도 못 끝냈을 때만 선다. 이 문구는
        // "오늘은 한두 개만 잡으셔도 충분합니다" — 하루를 여는 말이다.
        // 오후에 하면 이미 굴러간 하루 위에 얹는 말이 되고, 그 시간에는 곧
        // 시작할 일정이나 오늘 해낸 것을 짚는 편이 맞다. 늦게 잤어도 오전에
        // 두 개를 끝낸 사람에게 줄이자고 하는 것도 뒷북이라, 그때는 아래
        // 격려가 받는다. 한동안 이 갈래가 낮 슬롯 전체(7~17시)를 조건 없이
        // 가져가서, 오후 문구가 통째로 나가지 못했다.
        if (hour < 12 &&
            context.doneCount == 0 &&
            (context.feltSick ||
  ...
  /// 실행 유형에 맞는 처방 한 마디. 오늘 적어둔 계획 개수를 먼저 짚고 잇는다.
  ///
  /// 모르는 유형이면 null — 유형 이름은 앱이 세어 붙이는 것이라 여기 없는
  /// 이름이 올 일은 없지만, 새 유형을 만들고 문구를 안 채웠을 때 엉뚱한 말이
  /// 나가느니 조용히 넘어가는 편이 낫다.
  String? buildTypeAdvice(String type, {required int planCount}) {
    final pool = (planCount > 0
        ? voice.typeAdvice
        : voice.typeAdviceNoPlan)[type];
  ...
  ///
  /// 고른 답이 여럿이면 하나를 골라 쓰지 않고 함께 읽는다. 답을 하나씩 떼어
  /// 놓으면 "쉬는 리듬이니 그대로 두시라"로 끝나버리는데, 옆에 고른 답까지
  /// 보면 쉰 날들이 대개 그 조건이 무너진 날이라는 것이 드러난다. 그러면
  /// 건넬 말이 달라진다 — 쉬는 건 두고 그 조건을 챙기면 쉬어야 하는 날이
  /// 줄어든다고.
  String? buildConditionAdvice(List<String> answers, {required int planCount}) {
    if (answers.isEmpty) return null;

    final rest = answers.contains(restAnswer);
    final others = answers.where((a) => a != restAnswer).toList();
    if (rest && others.isNotEmpty) {
      final pool = voice.conditionRestPlusAdvice[others.first];
      final line = _adviceWithIntro(pool, planCount);
      if (line != null) return line;
    }

    // 조합에 쓸 문구가 없으면 답 하나짜리로 돌아간다. "일부러 쉬었어"만
    // 골랐을 때도 여기로 온다 — 그때는 쉬는 리듬을 그대로 두시라는 그 말이
    // 맞는 말이라, 따로 가를 것이 없다.
    for (final answer in [...others, if (rest) restAnswer]) {
      final line = _adviceWithIntro(voice.conditionAdvice[answer], planCount);
      if (line != null) return line;
    }
    return null;
  }

  /// 되는 조건이 아니라 안 되는 날을 두고 하는 답이라, 따로 이름을 둔다.
  static const String restAnswer = '일부러 쉬었어';
