...
class _RecordsScreenState extends State<RecordsScreen> {
  ...
  Future<String> _buildWeeklyFeedbackPrompt(int feedbackType) async {
    ...
    final executionFunnel = ExecutionFunnel.from(
      prefs.getString('nyang_history'),
    );
    final executionPatternBlock = executionFunnel.promptBlock();
    // 추세만은 실행 회고형도 받는다. 단계 비교까지 주면 무슨 회고든 유형
    // 이야기가 되지만, 더 해내고 있는지 아닌지는 어느 회고에서든 틀리면
...
${feedbackType == 0
   ...
   - "제일 많이 새는 곳"은 앱이 세 축을 견주어 고른 자리입니다. 그 자리를 그대로 쓰고, 다른 축을 병목으로 바꿔 부르지 마세요. 다만 지적하듯 짚지는 마세요 — 그 지점이 나아지면 전체가 어떻게 달라질지부터 격려하는 투로 말하고, 거기에 맞는 구체적인 방법을 하나만 제안하세요.
