...
3. [부정적 종결 금지]: 관련 일정이 부족하더라도 절대 "할 일이 없다"거나 "비전과 무관하다"며 단정적으로 대화를 끝내지 마십시오. 할 일이 아예 없을 때는 가볍게 비전 관련 15분 독서나 스트레칭 등 가벼운 생산적 행동을 제안하십시오.]''';
    } else if (userText == '오늘 핵심 추천') {
      effectiveUserText = '''오늘 핵심 추천
[System: 사용자가 방금 현재 시간 기준으로 "오늘 핵심 추천"을 요청했습니다. 시스템 프롬프트 상의 **최신 시간**과 **최신 할 일 현황(추가/완료 상태 등)**을 확인하여, 이전 대화 맥락에 얽매이지 말고 오늘 미완료된 할 일 중 가장 중요하게 챙겨야 할 핵심 할 일을 1~3순위 우선순위대로 추천해 주세요.

*데이터 분석 범위 및 연관성 판단 규칙 (1차 판단 원칙):*
오늘 핵심 일정을 추천(1~3개 선정)하기 전에, 오늘 등록된 할 일/일정/습관이 장기 비전, 마일스톤, 월목표, 주목표와 어떻게 연결되는지 아래 판단 원칙을 1차적으로 먼저 적용하십시오:
1. [키워드 겹침 판단]: 오늘 할 일 이름과 비전, 마일스톤, 월목표, 주목표 이름 사이에 겹치는 핵심 키워드(예: 'sns', 'ai', '공부', '영어', '코딩' 등)가 단 하나라도 존재한다면, 이 일정을 최우선적인 핵심 추천 후보(1순위)로 분류하고 선정합니다.
2. [연관 키워드 및 맥락적 연관성 판단]: 단어가 다소 다르더라도 개념적으로 서로 깊게 연관(예시: 장기 비전에 "시나리오 쓰기"가 있고 오늘의 할 일에 "영화 '헤어질 결심' 보기"가 있다면 연관된 일정으로 취급)되어 있다면 역시 핵심 추천 후보군으로 적극 고려하여 선정합니다.
3. [비연관/불확실 일정 처리 (침묵 원칙)]: 위 단계를 거쳤음에도 비전, 마일스톤, 월/주 목표와 연관성이 명확하지 않거나 아예 없는 일정(예: 단순 가사노동이나 무관한 일반 잡무 등)은 **오늘의 핵심 추천 대상에서 완전히 제외하고, 대화 피드백에서 아예 언급하지도 마십시오.**
   - 절대 "비전과 무관해서 추천에서 제외했다"거나 "비전 진행과 관계가 없어 추천하지 않는다"는 식의 부정적이고 단정적인 피드백을 주지 마십시오. 오직 비전/목표에 연결되는 1~3개의 핵심 일정과 그 구체적 추천 이유에만 초점을 맞춰 조언을 제안하십시오.

*출력 형식 규칙:*
1. 반드시 각 추천 항목마다 왜 이걸 핵심으로 잡아야 하는지 비전/목표와의 연결 고리를 이유로 한 줄씩 친절하게 쓰십시오.
2. 사용자가 특정 할 일을 언급하고 그걸 오늘 할 일로 등록할 만한 상황이 아니므로 [TASK] 태그는 절대 포함하지 마십시오.
3. 반드시 아래 형식으로만 답하십시오 (마크다운 ** 나 * 등은 절대 쓰지 마십시오):
   [CORE_REC:{"rank":1,"id":"할일id","text":"할일내용","reason":"추천이유"}]
   [CORE_REC:{"rank":2,"id":"할일id","text":"할일내용","reason":"추천이유"}]
   [CORE_REC:{"rank":3,"id":"할일id","text":"할일내용","reason":"추천이유"}]
   - 추천할 수 있는 핵심 일정이 3개 미만(예: 1~2개)이어도 전혀 상관없습니다. 형식 외 다른 캐릭터 고유의 조언 멘트는 앞에 한 줄만 가볍게 붙여서 출력하십시오.]''';
