...
- 사용자가 물은 것까지만 답하세요. 나가는 시각만 물었으면 준비 시각까지 밀어붙이지 마세요.'''
    ...
    // 제일 중요한 금지를 맨 위에 짧게 둔다. 프렌즈 코치가 약한 모델을 쓰던
    // 때에 맞춰 둔 자리다. 약한 모델은 긴 지시의 아래쪽을 흘려서, "보통
    // 30분이라 치면"처럼 없는 숫자를 지어냈다. 모델을 올린 뒤에도 순서는
    // 그대로 둔다 — 중요한 것을 앞에 두는 게 손해인 적은 없다.
...
- 흔적은 참고만 하세요. 사용자가 방금 한 말이 앱 기록보다 우선입니다.''',
  ...
  Future<bool> _startWeeklyConcretizeTip(
    ...
    final isFriendHabitStackCoach = _weeklyConcretizeFriendCoachIds.contains(
      widget.coachId,
    );
    if (!_coach.isMaster &&
        widget.coachId != 'cat' &&
  ...
  ///
  /// **최근 기록은 넘기지 않고, 오늘 목록만 넘긴다.** 이 사람이 어떤 모양으로
  /// 굴러가는지를 읽어 코칭하는 일은 기록 탭 몫이다
  /// ([_startWeeklyConcretizeTip] 설명 참고). 오늘 목록은 경계 안쪽이다 — 채팅이
  /// 맡은 것이 오늘이고, 알아두라고만 건네면 그 지식이 어디에 붙는지 알 수 없다.
  ///
  /// 목록이 비어 있으면 적을 때 쓰는 이야기가 된다. 아직 아무것도 안 적은 날이
  /// 오히려 이 이야기가 제일 잘 듣는 날이다.
  ///
...
[할 일]
아래는 앱이 기록에서 센 값이다. 이 기록을 근거로, 오늘 하루를 조금 더 알차게
보내려면 무엇을 어떻게 하면 좋을지 코칭해줘.
$block

[지킬 것]
- 지금 시각부터 할 수 있는 것만 말할 것.
- 오늘 이미 해둔 것은 감안해서 말할 것.
- 지난 날 한 일은 끝냈을 때 이미 축하를 건넸다는 걸 감안해서, 코칭 인사이트로만
쓸 것.
...
완성된 한 마디만 출력할 것. 다른 말은 덧붙이지 말 것.''';
  ...
  ///
  /// 한 시간이었는데 두 시간으로 늘렸다. 상태를 짚는 발화가 늘면서 방만
  /// 옮겨도 같은 말을 다시 듣는 일이 잦아졌다. 하루 총량을 깎는 대신 간격을
  /// 벌린 것은, 총량을 깎으면 저녁에 아예 조용해져서 챙겨준다는 느낌이
  /// 사라지기 때문이다. 핵심 일정 질문은 이 쿨다운을 건너뛰므로, 늘려도 꼭
  /// 물어야 하는 것은 그대로 나간다.
  static const _masterGreetingCooldown = Duration(hours: 2);
  ...
  Future<void> _loadHistoryAndGreet() async {
    ...
    // 인사를 고르기 전에 오늘 목록부터 맞춘다.
    //
    // 아래 발화들은 거의 다 오늘 목록을 보고 무슨 말을 할지 정한다. 그런데
    // 목록을 채우는 쪽은 앱이 켜질 때 기다림 없이 시작되므로, 인사가 먼저
    // 달리면 아직 안 채워진 목록을 보게 된다. 실제로 루틴 여덟 개를 등록해둔
    // 사람이 "오늘은 계획이 없구나" 쪽 인사를 받았다.
    //
  ...
  Future<String> _buildContextString(
    ...
    if (dsRaw != null && !needsGoalContext && !resolvedScope.pastChat) {
      try {
        ...
        if (onMind.isNotEmpty) {
          ...
          // "맥락을 파악하고" 같은 말은 안 쓴다. 대상을 찾아야 하는 동사는
          // 찾을 게 없는 날에 지어낸다. "이어질 때만"이 안 이어지면 쓰지 말라는
          // 뜻이라, 상관없는 날에 억지 연결을 만드는 것을 막는다.
          sb.writeln(
            '*며칠 사이에 사용자가 꺼낸 일들입니다. 오늘 이야기와 이어질 때만 쓰고, 무슨 이야기였는지까지 알아야 하면 [NEED: chat]으로 지난 날 요약을 받으세요.',
          );
        }
      } catch (_) {}
    }
    ...
    // 5-1. 오늘 쓸 수 있는 시간. 아침에 물어서 사용자가 직접 고른 답이다.
    //
    // 할 일 목록 바로 뒤에 둔다. 이 값이 뜻을 갖는 자리가 거기다 — 여섯 개가
    // 적혀 있는데 오늘 두세 시간뿐이라는 걸 같이 봐야 계획 이야기가 된다.
    //
    // 개수로 재던 것을 시간으로 바꾼 값이다. 두 시간밖에 없는 날의 다섯 개와
    // 온종일 비는 날의 다섯 개는 전혀 다른데, 개수는 그 차이를 못 본다.
    if (needsTaskContext) {
      ...
      if (capacity.isNotEmpty) {
        ...
        sb.writeln('- $capacity');
        sb.writeln(
          '*오늘 아침에 사용자가 직접 고른 답입니다. 짐작이 아니라 본인이 한 말이니, 오늘 계획을 이야기할 때 이 시간을 셈에 넣으세요.',
        );
        // 줄이라는 말부터 하지 않게 못박아 둔다. 시간이 모자란 것은 사용자가
        // 이미 아는 사실이고, 아침에 그렇게 답한 사람에게 "다 못 하실 것
    ...
    // 11. 취침 시간 (마스터 코치만)
    //
    // 플랜으로 가르는 것도 해봤다가 되돌렸다. 프렌즈 코치는 23시 고정 침묵으로
    // 이미 묶여 있어서([MasterGreetingContext.quietFromHour]), 이 사람이 몇 시에
    // 자는지를 알아도 하는 말이 거의 안 바뀐다. 잠을 챙기는 것은 마스터 몫이다.
    //
    // 못 내는 시간은 다르다. 그건 낮에도 걸리고 "지금 권하면 안 되는 시간"을
    // 가리키므로 프렌즈 코치에게도 실린다.
    if (_coach.isMaster && needsGoalContext) {
    ...
    if (_coach.isMaster && needsGoalContext) {
      ...
      if (isAllEmpty) {
        sb.writeln('\n[마스터 코치 참고 미완료 상태]');
        // 고정 일정도 함께 적는다. 한동안 빼두었는데, 설정 화면에 그 입력란이
        // 없어서 안내하면 없는 자리를 찾아 헤매게 되던 시절의 이야기다.
        sb.writeln('- 현재 사용자의 취침 예정 시간, 고정 일정, 장기 비전, 목표 등이 전혀 설정되어 있지 않습니다.');
        sb.writeln(
          '- 사용자가 "일정을 짜달라", "오늘 뭐부터 할까" 등 일정 관리와 관련된 대화를 시작할 때 한하여 자연스럽게 다음 내용을 덧붙여 유도하세요.',
        );
        sb.writeln(
          '- "설정 탭에서 [마스터 코치 참고]를 입력해 주시면, 제가 대표님의 생활 패턴에 맞춰 더 완벽하고 세밀하게 일정을 관리해 드릴 수 있습니다."',
        );
      ...
      if (shouldInterveneByLateEntry && minSleepTimeStr != null) {
        try {
          ...
          if (isLateNightEntry) {
            ...
            sb.writeln(
              '2. 사용자가 "있다" / "응" / "맞아" 등 긍정하면: 하려는 일을 막지 말고 짧게 받아 마무리.',
            );
            sb.writeln(
    ...
    //
    // 프렌즈에게도 준다. 한동안 마스터에만 줬는데, 일주일치 경향을 쥐여주면
    // 압박 없는 자리라는 전제가 깨진다는 이유였다. 그런데 이 유형은 기록
    // 탭에서 등급과 상관없이 이미 본인에게 보여주고 있다 — 배지와 코칭
    // 한마디까지 붙여서. 사용자는 자기가 계획 과다형인 걸 보고 있는데 코치만
    // 모르는 상태였던 셈이다.
    //
    // 프렌즈에 실행을 돕는 층이 없다는 것도 이유다. 오늘 목록은 받지만 그걸
    // 끝까지 가게 하는 재료가 없어서, 사용자가 먼저 말을 걸어야만 실행
    // 이야기가 시작됐다.
    //
...
- 원인이 불명확할 때만, 짧게 받은 뒤 아래 질문을 문장 그대로 한 번만 물으세요. 문장을 새로 만들거나 다른 질문을 덧붙이지 마세요.
  "$question"
- 답변은 2문장 이내로 유지하고 [TASK], [TIMER_CONFIRM] 태그를 출력하지 마세요.''';
  ...
  ///
  /// 등급으로도 나누지 않는다. 모든 코치가 답을 쓰기 전에 한 번 생각하고 쓰는
  /// 모델을 쓴다. 서버가 추론 세기를 낮게 걸어두어서 가벼운 말에는 거의 안 쓰고
  /// 무거운 상담일수록 더 쓴다.
  ///
  /// 프렌즈는 한동안 다른 모델이었다. 말이 길어지는 쪽이라 짧게 받아치는
  /// 캐릭터가 흐려질까 봐 뒀던 것인데, 값은 이유가 아니었다 — 이 앱은 긴
  /// 지시문과 이력이 매번 실려 입력이 훨씬 많고, 그쪽 단가는 이 모델이 오히려
  /// 싸다. 냥냥코치가 잡담보다 실무를 보는 자리라면 조금 늦고 조금 길어지는
  /// 편이 낫다고 보고 옮겼다.
  ///
  /// 되돌릴 일이 생기면 여기 한 줄이다. 다만 [AnalyticsService]의 모델별 단가도
  /// 함께 볼 것 — 모르는 이름은 제일 싼 값으로 떨어져서, 기록만 조용히 틀려진다.
  ...
  Future<String> _callOpenAI(
    ...
    if (shouldIncludeResistanceInterventionSection) {
      ...
      if (next == null) {
        ...
        //
        // 조각 목록을 싣던 자리다. 청소도 운동도 걷어냈다 — 이름과 효능을
        // 적어두면 코치가 그 열몇 개 안에서만 고르는데, 지금 모델은 그 사람이
        // 말한 상황에 맞는 것을 직접 고를 수 있다.
        //
        // 대신 한 줄만 남긴다. 개입이 든 예시("물건 하나 치우기")와 공통 대응의
        // "범위를 아주 작게"가 둘 다 짧고 위에 있어서, 그냥 두면 매번 제일
        // 작은 쪽으로 수렴한다. 구체적으로 짚으라는 말이 그걸 막는다.
        if (_isCleaningContext(userText) && next.picksConcreteAction) {
          interventionSection = '''$interventionSection
- 지금 이 사람 상황에 맞게 부담없이 시작하기 좋은 청소·정리 행동 하나를 구체적으로 언급해주세요.''';
        }
...
$resistanceFlowRule'''
    ...
    String assemble(String context, String contextRequestRule) {
      // 역할 서술 바로 뒤에 붙인다. 코치마다 적혀 있던 줄이라 원래 있던
      // 자리가 거기고, 이 앱이 하는 일을 말하는 줄이라 규칙 묶음보다 역할
      // 쪽에 붙어야 읽힌다.
