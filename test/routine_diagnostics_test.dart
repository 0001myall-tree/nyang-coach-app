...
void main() {
  ...
  test('그날 쉬기로 찍힌 것이 보인다', () {
    ...
    expect(text, contains('쉬기 (2026-09-05 09:12 찍힘)'));
  });

  test('쉬기가 언제 찍혔는지는 날짜까지 보여준다', () {
    // 시각만 적었을 때는 오늘 찍힌 것처럼 보였다. 실제로는 며칠 전에 미리
    // 찍힌 것이어서, 아직 오지도 않은 시각이 오늘 기록으로 나왔다.
    final text = report(
      habits: [
        {'id': 2, 'name': '영양제 챙겨먹기', 'freq': 'daily'},
      ],
      logs: {
        '2': {
          today: {
            'done': false,
            'status': 'skipped',
            'skippedAt': '2026-09-01T17:51:00.000',
          },
        },
      },
    );

    expect(text, contains('2026-09-01 17:51 찍힘'));
  });
  ...
  group('빠진 이유 짚기', () {
    ...
    test('오늘이 고른 요일이 아니면 그걸 짚는다', () {
      // 2026-09-05는 토요일(dbDow 5)이고, 고른 요일은 월수금이다.
      final reason = RoutineDiagnostics.missingReason(
        habit: {
          'id': 4,
          'freq': 'weekly',
      ...
      expect(reason, '오늘 요일이 고른 요일에 없음');
    });

    test('주 n회를 이미 채웠으면 그걸 짚는다', () {
      final reason = RoutineDiagnostics.missingReason(
        habit: {
          'id': 5,
          'freq': 'weekly_count',
        ...
        logs: {
          '5': {
            // 이번 주(월요일 2026-08-31 시작)에 두 번 채웠다.
            '2026-08-31': {'done': true},
            '2026-09-01': {'done': true},
          },
        },
        today: today,
      );
      expect(reason, '이번 주 횟수를 이미 채움');
    });
  });

  test('주 n회 루틴은 이번 주에 몇 번 했는지 같이 보여준다', () {
    final text = report(
      habits: [
        {
          'id': 5,
    ...
    expect(text, contains('이번 주(오늘 빼고): 1 / 3회'));
  });

  test('오늘 목록에 없는 핵심을 짚어준다', () {
    final text = report(
      tasks: [
    ...
    expect(text, contains('앱 개발 (오늘 목록에 없음 ← 어제 것이 남은 자리)'));
    expect(text, contains('sns 글쓰기 (오늘 목록에 있음)'));
  });
}
