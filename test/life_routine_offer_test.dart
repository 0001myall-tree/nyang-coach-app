...
/// 부르는 쪽은 통신이라 여기서 돌릴 수 없다. 받아온 말을 다듬는 부분만 본다.
///
/// 이 말은 코치 답변을 거치지 않고 그대로 화면에 나간다. 태그가 남아 있으면
/// 사용자에게 대괄호가 그대로 보인다.
void main() {
  group('받아온 말 다듬기', () {
    test('그냥 말이면 그대로', () {
    ...
    test('태그는 떼어낸다', () {
      expect(
        LifeRoutineOffer.clean('토요일 오전에 청소 20분 어때? [TASK: 청소]'),
        '토요일 오전에 청소 20분 어때?',
      );
    });
    ...
    test('태그만 오면 아무 말도 안 한 것으로 본다', () {
      expect(LifeRoutineOffer.clean('[TASK: 청소]'), isNull);
    });
