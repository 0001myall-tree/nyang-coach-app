...
class MainActivity : FlutterFragmentActivity() {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, morningAlarmChannel)
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    ...
                    "readAlarmVolume" -> {
                        result.success(readAlarmVolume())
                    }
                    "setAlarmVolume" -> {
                        ...
                        result.success(setAlarmVolume(level))
                    }
    ...
    private fun startMorningAlarmSound(soundName: String?) {
        stopMorningAlarmSound()
        // 알림에 붙은 목소리가 아직 울리는 중이다. 여기서부터는 앱이 같은
        // 목소리를 반복 재생하므로, 두 소리가 살짝 어긋나게 겹쳐서 들린다.
        // 알림을 지우면 그쪽 소리도 멎는다.
        (getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager)
            .cancel(MorningAlarmReceiver.NOTIFICATION_ID)
