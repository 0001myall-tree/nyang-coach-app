    ...
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray, widgetData: SharedPreferences) {
        appWidgetIds.forEach { widgetId ->
            val views = RemoteViews(context.packageName, R.layout.cat_character_widget_layout).apply {
                ...
                val showsCoreDoneMessage = displayKind == "core_done"
                val showsCompletionMessage = displayKind == "cheer" && progress >= 100
                val showsStatusRow = status.isNotEmpty() && (displayKind == "timed" || displayKind == "habit" || displayKind == "core" || displayKind == "in_progress")
                ...
                if (showsCompletionMessage) {
                    ...
                    setViewVisibility(R.id.cat_character_status_row, View.GONE)
                    setViewVisibility(R.id.cat_character_paw_row, View.VISIBLE)
                    setTextViewText(R.id.cat_character_text, "핵심 완료했다냥!")
                    setPawProgress(this, pawCount, progress)
